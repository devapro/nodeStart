...
describe('API Auth', function() {
    it('Test error login', function(done) {
        request(url)
            .post('/api/login')
            .send({"id":12,"params":{"email":"user@dfdf.df","password":"23212"}})
    ...
    it('Test success login', function(done) {
        request(url)
            .post('/api/login')
            .send({"id":12,"params":{"email":"user@example.com", "password":"123456"}})
            .end(function(err, res) {
                if (err) {
                    throw err;
                }
                ...
                res.body.result.data.items.should.have.property('token');
                res.body.result.data.items.should.have.property('role');
                done();
            });
    });
});
