...
const SysModel = require('../../../models/SysModel').SysModel;

const AjaxResponseObject = require('../../../components/AjaxResponseObject');

/* listing. */
router.get('/', function (req, res, next) {
    res.render('layouts/admin/list', { title: 'Log Sys', dataUrl: "/admin/sys/sys/list/" });
});
...
router.get('/list/:perPage/:page', function (req, res, next) {
    ...
    json.urls = {
        "view": "/admin/sys/sys/view/",
        "update": false,
        "delete": "/admin/sys/sys/delete/",
        "delete_all" : "/admin/sys/sys/delete_all/"
    };
    json.data = [];
...
router.get('/view/:id', function (req, res, next) {
    var response = new AjaxResponseObject();
    SysModel.findById(req.params.id, function (err, doc) {
        if(err){
            log.error(err);
            res.statusCode = 500;
            response.addError(err.message);
...
router.get('/delete/:id', function (req, res, next) {
    SysModel.findByIdAndRemove(req.params.id, function (err) {
        if(err ){
