...
router.get('/', function (req, res, next) {
    res.render('layouts/admin/list', { dataUrl: "/admin/sys/errors/list/" });
});
...
router.get('/list/:perPage/:page', function (req, res, next) {
    ...
    json.urls = {
        "view": "/admin/sys/errors/view/",
        "update": false,
        "delete": "/admin/sys/errors/delete/",
        "delete_all" : "/admin/sys/errors/delete_all/"
    };
    json.data = [];
...
router.get('/view/:id', function (req, res, next) {
    var response = new AjaxResponseObject();
    LogErrorModel.findById(req.params.id, function (err, doc) {
        if(err){
            log.error(err);
            res.statusCode = 500;
            response.addError(err.message);
...
router.get('/delete/:id', function (req, res, next) {
    LogErrorModel.findByIdAndRemove(req.params.id, function (err) {
        if(err ){
...
router.get('/delete_all', function (req, res, next) {
    LogErrorModel.remove(function (err) {
        if(err ){
            return res.status(404)        // HTTP status 404: NotFound
                .send('Not found');
        }
